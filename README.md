...
Personal portfolio site of **Nikita Boyarkin** — Product / Data Analyst. Bilingual (RU / EN), static, zero-JS-by-default.

**Live:** https://nikitaboyarkin.github.io/ · **EN:** https://nikitaboyarkin.github.io/en/
...
| Analytics | PostHog (`posthog-js`), inert when the build-time key is unset |
| Hosting | GitHub Pages via GitHub Actions |
| Charting | Native SVG chart components + `src/lib/chart-svg.ts` (no chart library) |

## Requirements

- **Bun** (only package manager used here — `bun.lock` is the lockfile)
...
**17 case studies**, each with a page at `/projects/<slug>/` (RU source + EN mirror), a board column set by `track`, and a public repo and/or live demo where one exists. Three are the **headline cases** featured on the home page — `volta`, `sql`, `cohort` — chosen because each is backed by a reachable public artifact.
...
Run this trio after every content or component change. All three green = safe to commit.
...
│   ├── pages/              # routes + endpoints (rss, sitemap, robots, llms.txt, search-index.json)
│   ├── data/               # chart data + generated github-activity.json
│   └── styles/             # global.css (tokens + themes), blog.css
├── scripts/                # OG/CV generators, GitHub sync, check_site.py, content-drift-audit
...
└── public/                 # images, hero SVGs, OG images, fonts, CV PDF, demos
```
...
| `graph.ts` / `graph-layout.ts` / `graph-tooltip.ts` / `graph-zoom.ts` / `graph-url.ts` | Build-time knowledge graph: greedy-modularity communities, force layout, interaction |
...
Old routes collapse into the `/about` anchor cluster (`astro.config.mjs`): `/whois/`, `/work-with-me/`, `/now/`, `/start/` → `/about/#…`; `/guides/` → `/notes/guides/`; `/cv/` → the CV PDF.

## Analytics

`src/components/Analytics.astro` renders the PostHog snippet. It is **inert unless build-time env vars are set**:

| Variable | Purpose |
|---|---|
| `PUBLIC_POSTHOG_KEY` | Project token — no snippet rendered when unset |
| `PUBLIC_POSTHOG_HOST` | Defaults to `https://us.i.posthog.com` |
| `PUBLIC_BEACON_ENDPOINT` | Optional — pushes scroll-depth / RED beacon events to the monitoring stack |
