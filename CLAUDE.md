...
# Run lib unit tests (bun:test) — no build required
bun test

# Dist-dependent tests (read the real built HTML in dist/)
bun run test:built
...
1. Installs Bun dependencies (`bun install --frozen-lockfile`).
2. Checks GitHub <-> portfolio drift (`bun run sync:gh`, exits 1 on hard drift).
