...
caseStudy:
  ...
  result: "LightGBM и логистика почти вровень по AUC (0.904 против 0.917), но LightGBM заметно лучше калиброван (Brier 0.068 против 0.099). В топ-10% самых рискующих модель ловит 53% реальных отточников — lift 3.07×. Сильный recency-признак делает lift скромным; ценность проекта — в схеме без утечек и в бизнес-метрике."
  metrics:
    - label: "Recall@top-10%"
      value: "0.53"
    - label: "Lift@top-10%"
