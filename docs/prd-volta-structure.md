...
| Field | Value |
|---|---|
| Status | Phases 0–5 executed 2026-09-20 (§10). |
| Date | 2026-09-20 |
...
| D10 | **Hub layout (moderate):** `Итог в 30 секундах → Дело → Карта проекта → Улики №1–4 → Вердикт → Графики → Остальные проекты → Слой RAT v2 → Рекомендации и гейты → Данные и метод → Эффект → Документация`. Curated hub charts = `onboarding-funnel`, `kyc-ab-conversion`, `cohort-retention-heatmap`, `retention-pre-post`, `segmentation-size-vs-revenue`, `segmentation-pareto-cumulative`. **Amended 2026-09-26:** the skeleton headings follow STAR — actual H2 order is now `Итог в 30 секунд → Ситуация → Задача → Действия → Карта проекта → Улики №1–4 → Слой RAT v2 — валидация собственного аудита → Рекомендации и гейты → Вердикт → Ограничения → Остальные проекты → Документация` (12 H2, RU; EN mirrors). `## Вердикт` — the R — stays **after** `## Рекомендации и гейты`, i.e. out of STAR order: **intended, not enforced**, kept for the narrative. | Fixes P1 without discarding the narrative. The order is pinned by `tests/lib/content-skeleton.test.ts`; STAR ordering *inside* the hub deliberately is not. |
| D11 | **Legitimize the parts "dossier" skeleton** (`Контекст → Данные и метод → Выводы → Рекомендации → Документация`); fold the 4 one-off `Визуализация` into `Данные и метод`; **amend `prd-readability.md` D11** (projects keep the shared skeleton; parts get the dossier skeleton). | The parts' convention is already 100% uniform and reads better for a dossier. Renaming 66 headings to satisfy a stale rule has no reader benefit. **Amended 2026-09-26:** the dossier becomes **STAR** — `Ситуация → Задача → Действия → Результат → Рекомендации → Документация` (`Situation → Task → Actions → Result → Recommendations → Documentation`); `Рекомендации` is retained as the sixth section. Rename map: `Контекст`→`Ситуация`, `Данные и метод`→`Действия`, `Выводы`→`Результат`; a new `Задача` was authored per file. `docs/prd-readability.md` D11 amended in turn. |
...
| 2 | **Done 2026-09-20.** Hub restructured to D10 (RU+EN): `Итог в 30 секундах → Дело → Карта проекта → Улики №1–4 → Вердикт → Остальные проекты → Слой RAT v2 → Рекомендации и гейты → Данные и метод → Эффект → Документация`. Map generated from the collection by `bun run volta:map` (`scripts/gen-volta-map.mjs`). Hub charts 34 → **6** via `HUB_CHART_IDS`; `## Визуализации` removed; `Досье` + `Ключевые результаты` collapsed into the map; demo link added; the flat `children` list removed (the map supersedes it). **Verification:** `build` 135 pages; `check` 0/0/0; hub `id="chart-"` count 6 (RU+EN); `audit:content` green after re-snapshot. |
| 3 | **Done 2026-09-20.** Skeleton: the 4 one-off `Визуализация` blocks folded; **0** remain across 46/46 files. Navigation: layer label + prev/next by `order` + layer-grouped map on all 23×2 part pages. Result-summary lead block added. Visualization: 6 PNGs converted to native charts (`segmentation-pca-scatter`, `segmentation-k-selection`, `churn-roc-curves`, `churn-shap-importance`, `anomaly-hour-amount`, `traveler-fx-sensitivity`), with data dumped **read-only** from `volta-banking` using the same seeds and committed CSVs (a throwaway dump script, not added to the source repo); SHAP-local kept as the single PNG; the orphan `segmentation_silhouette.png` folded into the k-selection chart and deleted. `ChartScatter` extended for grouped colour + legend. Volta PNGs in content 7(+1 orphan) → **1**; `scatter` used 2×; `volta.json` 36 → **42** charts. **Verification:** `build` 135 pages; `check` 0/0/0; `audit:content` green. |
| 4 | **Done 2026-09-20.** Featured card (RU+EN) now shows 3 `caseStudy` metrics + demo + GitHub chips. prev/next fixed: both locales use `PROJECT_ORDER` (was a stale 8-slug local list). Dead `ProjectSpotlight.astro` / `HomeBoard.astro` removed. Demo rebuilt: `17 → 23` projects, `170+ → 264` tests (measured: `pytest` → 264 passed, 98% coverage), causal KPI `+9.2pp → +9.09pp`, referral KPI `36.0% → 29.6%`; demo linked from the hub. Sibling cross-links added via `## Остальные проекты`. **Verification:** `build` 135 pages; featured-card metrics/chips present RU+EN; prev/next on Volta → `ab`. |
| 5 | **Done 2026-09-20.** Gates: `build` 135 pages; `check` 0/0/0; `audit:content` green (baseline re-snapshotted: 106 files / 4035 tokens); `bun test` 153 pass / 1 pre-existing unrelated fail (`metrics.test.ts:39`, sqlCases 25 vs 26 — reproduces with the changes stashed). Docs: `CLAUDE.md` (parts dossier skeleton, `layer`, hub map + `volta:map`, curated charts), `docs/prd-readability.md` D11 amended, `CONTEXT.md` v8 terms. **Deferred (honest):** D13 key-finding annotations on the line/scatter SVG components (bar value labels already existed). |
| 6 | **Done 2026-09-26 — STAR amendment.** Hub (RU+EN): `Задача`→`Ситуация`, `Goal`→`Situation`; a new `Задача`/`Task` inserted; `Данные и метод`→`Действия`, `Data & Method`→`Actions`. All 23×2 parts: `Контекст`→`Ситуация`, `Данные и метод`→`Действия`, `Выводы`→`Результат` (+ EN twins) and a new `Задача`/`Task` each. `## Рекомендации` retained per D11. The hub's final H2 order is recorded in D10. **Verification:** `bun test tests/lib` 220 pass / 0 fail — `content-skeleton.test.ts` pins both the parts skeleton and the exact hub order; `check` 0 errors; `build` 135 pages; `audit:content` no drift. Deep links to the retired heading anchors are filed as PENDING in `docs/prd-readability.md` §8. |
